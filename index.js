...
// // Middleware
// app.use(cors());
// app.use(express.json()); // Ensure this is before your route definitions
...
// // Creating endpoint to get app data
...
const cors = require("cors");
const error = require("mongoose/lib/error");
// Removed the import statement for userEvent
const jwt = require("jsonwebtoken");
require('dotenv').config();
...
app.post('/signup', async (req, res) => {
    ...
    const token = jwt.sign(data, process.env.SECRET_KEY); // Fixed secret key reference
    res.json({ success: true, token });
});
...
const fetchUser = async (req, res, next) => {
    ...
    if (!token) {
        ...
        try {
            const data = jwt.verify(token, process.env.SECRET_KEY); // Fixed secret key reference
            req.user = data.user;
            next();
        } catch (error) {
            res.status(401).send({ errors: "Please authenticate using a valid token" });
